...
if not filled:  # facciamo i punti del perimetro
    ...
    xy = np.random.randint(
        0, 2, size=N * S
    )  # genero un array di lunghezza N*S contenente 0 or 1
    ...
    side = np.random.randint(
        0, 2, size=N * S
    )  # genera un array di lunghezza N*S contenente 0 or 1
    ...
    a = np.random.uniform(0, 1, size=S * N) * xy + side * (
        1 - xy
    )  # coordinata orizzontale
    b = (
        np.random.uniform(0, 1, size=S * N) * (1 - xy) + side * xy
...
width = np.repeat(
    np.random.uniform(3, 10, size=N), S
)  # np.repeat ripete per S volte un array di dimensione N
...
if filled:
    radius = (
        np.random.uniform(0, 1, S * N) ** 0.5
    )  # per rendere uniforme il raggio al quadrato
...
X = np.concatenate(
    [X_rect, X_circ]
)  # come usare np.stack con axis = 0, cioè impila con righe
...
dataset_train = [
    torch_geometric.data.Data(
        x=torch.Tensor(x[: random.randint(S / 2, S)]), y=torch.Tensor(y.reshape(1, 1))
    )
...
for j in range(3):
    ...
    nx.draw(
        grafo,
        {i: [p[0], p[1]] for i, p in enumerate(x)},
        node_size=(25 if filled else 100),
    )
...
class DNNMPLayer(
    MessagePassing
):  # MessagePassing è una classe che gestisce il passaggio di info tra i neuroni del grafo

    def __init__(
        self, in_channels, out_channels
    ):  # in_channels sarà 2 poichè ogni dato è specificato da 2 coordinate (x,y)
        ...
        super().__init__(
            aggr="max"
        )  # imponiamo la funzione di aggregazione come il massimo
    ...
    def message(self, h_j, h_i):  # funzione chiamata da propagate numerose volte
        ...
        input = torch.cat(
            [h_i, h_j - h_i], dim=-1
        )  # cat sta per concatenate e li incolla in orizzontale (h_i, h_j - h_i)
        # poichè è impostato con dim = -1

        return self.mlp(
            input
        )  # il messaggio che dovrò passare e quello prodotto dalla mlp con i dati di input qui definiti
...
class MessagePassingNetwork(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.pass1 = DNNMPLayer(
            2, N_features
        )  # creo una istanza dell' oggetto DNNMP layer che ha dentro un MLP
        # che lavora prendendo in input 2 dati e producendone 20

        self.pass2 = DNNMPLayer(
            N_features, N_features
        )  # faccio comunicare un altra volta i nodi con i suoi collegamenti
        # questa ogni volta ogni nodo ha 20 features perchè risultanti dalla precedente azione di pass1

        self.pass3 = DNNMPLayer(
            N_features, N_features
        )  # facciamo una ultima comunicazione tra nodi

        self.classifier = Linear(
            N_features, 1
        )  # produco un unico output perchè devo saper distinguere i cerchi dai rettangoli
    ...
    def forward(self, data, batch):  # data sarà il dataset che devi dare al grafo
        ...
        h_global = global_mean_pool(
            h, batch
        )  # prende tutte le features di tutti i nodi del grafo di tutti i grafi "h" e per ogni grafo
...
)  # con dataset_train definito come tensore torch sopra
...
)  # organizza il dataset di training in batch di 10 grafi
...
def train(model, optimizer, loader):
    ...
    for data in loader:  # loader è il batch di grafi da analizzare nel training verrà
        ...
        out, out_nodes = model(
            data, data.batch
        )  # chiama il forward del modello MessagePassingNetwork
        ...
        loss = lossFunc(
            out, data.y
        )  # calcolo loss, data.y sono le etichette 0,1 in base a che siano cerchi o rettangoli
        ...
        )  # l' errore nel batch è data dallo loss di ogni grafo per il numero di grafi

    return total_loss / len(
        loader.dataset
    )  # per ottenere la loss media sul singolo grafo
...
def test(model, loader):  # CREIAMO LA FUNZIONE DI TEST
    ...
    for data in loader:
        ...
        pred, pred_nodes = model(
            data, data.batch
        )  # pred è la predizione totale sui 10 grafi del batch
        ...
        total_correct += int(
            ((pred > 0.5) == data.y).sum()
        )  # data.y è 0 or 1 e lui lo setta in automatico a True or False
...
for epoch in range(1, 10):
    ...
    )  # ricordiamo che la funzione training restituisce la loss "media" del grafo

    test_acc, test_loss = test(
        model, test_loader
    )  # ricordiamo che la fuzione test restituisce la loss "media" del grafo e il numero
    ...
    print(
        f"Epoch: {epoch:02d}, Train Loss: {loss:.4f}, Test Loss: {test_loss:.4f}, Test Accuracy: {test_acc:.4f}"
    )
