...
x,y = make_blobs(n_samples = 100, centers = 2, random_state = 0, cluster_std = 0.60) #creiamo 50 punti suddivisi in 2 gruppi
...
def plot_svc_decisione(model, ax = None, plot_support = True):
    ...
    xlim = ax.get_xlim() #con questo imposto xlim uguale al bordo x dell' area su cui sta lavorando
    ylim = ax.get_ylim() #e con questo imposto il bordo y 
    
    # create grid to evaluate model
    x_lin = np.linspace(xlim[0], xlim[1], 30) #creo 30 punti equispaziati sull' asse x a partire da
    y_lin = np.linspace(ylim[0], ylim[1], 30) 

    Y, X = np.meshgrid(y_lin, x_lin) #creo una griglia 30x30 cioè 900 punti, che copra l' intera area del grafico
