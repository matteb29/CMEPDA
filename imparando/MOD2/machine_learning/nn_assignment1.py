...
layer_input = Input(
    shape=(2,)
)  # numero di neuroni iniziali è uguale al numero di feature: quindi il numero di colonne
...
print(
    f" Chiavi del dizionario restituito da rete.fit: {risultati_training.history.keys()}"
)
...
testSet = np.stack(
    (xx1.flatten(), xx2.flatten()), axis=1
)  # axis = 1 li impila come colonne, axis = 0 li avrebbe impilati come colonne
...
print(
    f"Risultato della predizione della rete sul primo dato del test set: {predizioni[0]}"
)  # questo contiene delle probabilità perciò facciamo np.round,
...
plt.scatter(matrix[:, 0], matrix[:, 1], c=y, cmap=cm_bright, edgecolors="k")
plt.contourf(xx1, xx2, predizioni.reshape(xx1.shape), cmap=plt.cm.RdBu, alpha=0.8)
