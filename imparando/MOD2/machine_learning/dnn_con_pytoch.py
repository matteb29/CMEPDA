...
plt.contourf(xx1, xx2, (funzione(xx1,xx2)).reshape(xx1.shape), cmap = plt.cm.RdBu, alpha = .8)
...
class rete_neurale_prova(nn.Module):
    ...
    def __init__(self):
        ...
        self.layer1 = nn.Linear(2,50) #2 perchè ho 2 dati in ingresso, 50 è il numero di output corrispondente al numero di neuroni
...
for epoch in range(numero_epoche): #ciclo for per le epoche
    ...
    for i in range(0, len(X), batch_size): #ciclo for per ogni epoca per ottimizzare i parametri
    ...
    print(f"finita epoca numero: {epoch}, ultimo valore loss {loss_batch}, tempo di esecuzione: {time.time() - start_time} sec")
...
plt.contourf(xx1,xx2, predizioni_test.reshape(xx1.shape), cmap = plt.cm.RdBu, alpha = .8)
