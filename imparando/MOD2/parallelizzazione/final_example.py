...
def factorize_naive(n):
    ...
    while True:
        ...
        if r == 0:
            ...
            n = (
                n // p
            )  # questo fa la divisione intera togliendo la parte decimale e arrotondando
...
def threaded_factorizer(nums, nthreads):
    ...
    def worker(nums, outdict):
        ...
        for (
            n
        ) in nums:  # ricorda che nums è una lista di numeir che vogliamo fattorizzare
    ...
    for i in range(nthreads):
        ...
        t = threading.Thread(
            ...
            args=(
                nums[chunksize * i : chunksize * (i + 1)],
                # chunksize * i =  start
...
def mp_factorizer(nums, nprocs):
    ...
    for i in range(nprocs):
        ...
        p = multiprocessing.Process(
            target=mp_worker, args=(nums[chunksize * i : chunksize * (i + 1)], out_q)
        )
